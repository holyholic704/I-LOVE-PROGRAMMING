...
- int：保存整数值，且范围在 long 类型之内，即 `-2^63 ~ 2^63 - 1`，超过则按字符串处理
  - 浮点数使用字符串保存
- embstr：保存字符串，且 `字符串长度 ≤ 32 字节`
- raw：保存字符串，且 `字符串长度 ＞ 32 字节`
...
- Redis 7.0 之前：**hashtable 和 ziplist**
- Redis 7.0 之后：**hashtable 和 listpack**

元素数量大于 `hash-max-listpack-entries`，默认值为 512，或元素大小大于 `hash-max-listpack-value`，默认值为 64 字节，由 listpack 改为 hashtable
...
- Redis 7.2 之前：**hashtable 和 intset**
- Redis 7.2 之后：**hashtable、intset、listpack**

当集合内只包含整数值元素，且元素数量小于 `set-max-intset-entries`，默认值为 512，使用 intset

当集合内元素数量大于 `set-max-listpack-entries`，默认值为 128，或元素大小大于 `set-max-listpack-entries`，默认值为 64 字节，由 listpack 改为 hashtable
...
当集合内元素数量大于 `zset-max-listpack-entries`，默认值为 128，或元素大小大于 `zset-max-listpack-entries`，默认值为 64 字节，由 listpack 改为 skiplist
