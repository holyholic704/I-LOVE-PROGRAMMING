...
- Redis 7.0 之前：**ziplist 和 hashtable**
- Redis 7.0 之后：**listpack 和 hashtable**

元素数量超过 `hash-max-listpack-entries`，默认值为 512，或元素大小超过 `hash-max-listpack-value`，默认值为 64 字节，由 listpack 改为 hashtable
...
- Redis 7.2 之前：**intset 和 hashtable**
- Redis 7.2 之后：**intset、listpack、hashtable**
