...
- 读未提交（READ UNCOMMITTED）：最低的隔离级别，允许读取尚未提交的数据，可能会导致脏读、幻读、不可重复读
  - 一般只是理论上存在，数据库的默认隔离级别都高于该级别
- 读已提交（READ COMMITTED）：允许读取已提交的数据，可以避免脏读，还是可能发生幻读、不可重复读
...
ReadView 用于判断当前事务可以看见的版本，有四个主要的字段
...
事务开启时，会先使用快照读。在使用快照读时，如果当前事务修改了其他事务插入的记录，仍会发生幻读。在使用当前读时，是不会发生幻读的，所以尽量在开启事务之后，马上执行 `select ... for update` 这类当前读的语句
