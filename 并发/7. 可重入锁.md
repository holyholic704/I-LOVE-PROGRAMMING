...
- ReentrantLock 可以响应中断，而 synchronized 只能等待其执行完成，或者在内部监听中断状态
...
通过源代码可以看出各个方法内部或多或少都依靠着 Sync 类实现功能
...
ReentrantLock 主要依靠内部类 Sync 实现加锁解锁等操作，而 Sync 又是基于 AQS 实现的
...
static final class FairSync extends Sync {
    ...
    // 与Sync类中的nonfairTryAcquire方法类似，只多了检查是否有等待队列的操作
    protected final boolean tryAcquire(int acquires) {
        final Thread current = Thread.currentThread();
        int c = getState();
        // 判断当前是否有线程持有该锁
        if (c == 0) {
            // 如果当前没有等待队列，或者有等待队列但当前线程就是下一个要获取锁的线程（不是头结点，而是头结点的后继节点），且通过CAS操作成功获取到锁，如果是就不需要进队列了
            // 如果有等待队列，且等待队列中的节点目前都没有持有锁，会发生争抢
...
public final boolean hasQueuedPredecessors() {
    ...
    // 所以当前队列不为空且头结点的后继节点就是持有当前线程的节点，才会返回true
    return h != t &&
